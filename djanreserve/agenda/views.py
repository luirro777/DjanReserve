...
class ProximasReservasView(ListView):
    ...
    def get_queryset(self):
        ...
        return Agenda.objects.filter(
            lugar_id=lugar_id, fecha__gte=timezone.now()
...
class AgendaCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        if Agenda.objects.filter(
            lugar=lugar,
            fecha=fecha,
            hora_comienzo__lt=hora_final,
            hora_final__gt=hora_comienzo,
        ).exists():
            form.add_error(
                "hora_comienzo",
                ValidationError(
                    _("El lugar ya está reservado en este período de tiempo.")
                ),
            )
...
class AgendaDeleteView(DeleteView):
    model = Agenda
    template_name = "agenda/agenda_confirm_delete.html"
    success_url = reverse_lazy("agenda_list")
